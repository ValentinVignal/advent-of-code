...
const results = bluePrints.map((bluePrint, index) => {
  ...
  const run = (state: State): number => {
    ...
    const geode = minuteState.robots.geode;
    ...
    const result = geode + Math.max(...scenarios.map(
      (scenario) => {
        return run(scenario);
      }),
    );
...
const result = results.reduce((acc, cur, index) => acc + (cur * (index + 1)), 0);
