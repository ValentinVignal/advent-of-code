...
class RepairDroid {
  ...
  fillOxygenSystem(grid: Map<string, OutputType>): number {
    type QueueItem = XY & { distance: number };

    const queue: QueueItem[] = [
      ...[...grid.entries()]
        .filter(([, value]) => value === OutputType.OxygenSystem)
        .map(([key]) => {
    ...
    while (queue.length) {
      ...
      for (const [dx, dy] of [
        ...
        const newX = x + dx;
        const newY = y + dy;
        const newKey = xyToString({ x: newX, y: newY });
        if (grid.get(newKey) === OutputType.Empty && !visited.has(newKey)) {
...
gridPromise.then((grid) => {
  const result = repairDroid.fillOxygenSystem(grid);
