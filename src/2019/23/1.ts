...
class Computer {
  ...
  static resultPacket: Packet | null = null;
  ...
  private program = new Program(
    ...
    async (output) => {
      if (!this.emittedPacket) {
        ...
        if (this.emittedPacket.destination === 255n) {
          Computer.resultPacket = this.emittedPacket as Packet;
        } else {
...
const main = async (): Promise<void> => {
  const computers = Array.from(
    { length: 50 },
    (_, index) =>
      new Computer(BigInt(index), (value) => {
  ...
  const result = Computer.resultPacket!.y;

  console.log(result); // 21160
};
