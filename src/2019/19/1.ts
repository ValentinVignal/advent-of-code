...
const getAllPositions = function* (width: number): Generator<XY> {
...
const main = async (): Promise<void> => {
  ...
  const grid: ("." | "#")[][] = Array.from({ length: width }, () =>
    Array.from({ length: width }, () => "." as "." | "#"),
  );

  for (const position of getAllPositions(width)) {
    let xOrY: keyof XY = "x";
    ...
    const program = new Program(
      ...
      () => {
        if (xOrY === "x") {
          xOrY = "y";
          return Promise.resolve(BigInt(position.x));
        } else {
          xOrY = "x";
          return Promise.resolve(BigInt(position.y));
        }
      },
    );
    await program.run();
