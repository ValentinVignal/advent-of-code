...
class Game {
  ...
  program = new Program(
    (output) => {
      switch (this.outputType) {
        ...
        case OutputType.TileId:
          if (this.currentOutput.x === -1n && this.currentOutput.y === 0n) {
            this.state.score = output;
          } else {
            switch (output) {
              case 3n:
                this.state.horizontalPaddle = { ...this.currentOutput };
                break;
              case 4n:
                this.state.ball = { ...this.currentOutput };
                break;
            }
          }
    ...
    () => {
      if (!this.state.ball || !this.state.horizontalPaddle) {
        throw Error("Ball or horizontal paddle position not known");
      }
      return BigInt(
        Math.sign(Number(this.state.ball.x - this.state.horizontalPaddle.x)),
      );
    },
  );
