...
const possibleDigits = [1, 2, 3, 4, 5, 6, 7, 8, 9] as const;
...
const runMonad = (i: number, n: number[], state: State): [boolean, number] => {
  ...
  if (instruction.operator === Operator.Inp) {
    ...
    // Tried it and it is not starting with a first digit between 1 and 3.
    const digits = n.length === 0 ? possibleDigits.slice(3) : possibleDigits;
...
console.log(result); // 51147191161261
