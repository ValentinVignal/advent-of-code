...
const evaluate = (packet: Packet): number => {
  ...
  switch (packet.type) {
    case 0:
      return packet.subPackets.reduce((acc, subPacket) => acc + evaluate(subPacket), 0);
    case 1:
      return packet.subPackets.reduce((acc, subPacket) => acc * evaluate(subPacket), 1);
    case 2:
    ...
    case 7:
      return evaluate(packet.subPackets[0]) === evaluate(packet.subPackets[1]) ? 1 : 0;
    default:
      throw new Error(`Unknown type ${packet.type}`);
  }
}

const result = evaluate(packetTree);

console.log(result); // 1114600142730
